---
layout: post
title: "SQL query 위주의 DAG jobflow를 EMR pyspark code로 구현"
...
1) Job이 이전까지 실행된 sql과 더불어 가장 마지막의 거대한 sql을 한번에 실행하는 구조이기에 성능&디버깅을 트래킹하기 어려운 구조로 spark dataframe 관점에서 성능적인 튜닝이 필요함


2) Data를 read 하는 job말고, sql query job에 대해서 특정 job의 dataframe이 다른 job에서 여러번 참조할 경우 lineage를 중간에 단절시키는 목적으로 해당 job의 dataframe을 hdfs에 임시로 save한 다음 다시 로딩해서 사용할 수 있도록 처리


** lineage가 길어질수록 연산량이 많이지기 때문에 HDFS에 저장시킴으로써 lineage를 단절시켜 연산의 오버헤드를 감소시킴


3) 여러번 참조되는 일부 dataframe은 cache처리
