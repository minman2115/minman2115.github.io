...
#### localPC --> S3 --> EC2(Fluent agent) --> SQS --> Kinesis streams --> Lambda --> Elasticsearch --> kibana --> User
...
def get_computer_metric():
    ...
    computer_stat['timestamp'] = (datetime.datetime.now().strftime('%Y/%m/%d %H:%M:%S'))
...
while True:
    ...
    if count == 60:
        ...
        s3.Bucket('iac-mng').put_object(Key='iac_realtime_architecture/'+filename+'.csv',Body=data)
        data.close()
...
[ec2-user@ip-10-88-0-200 etc]$ sudo vim /etc/td-agent/td-agent.conf
  <source>
    ...
    <parse>
      @type csv
      keys timestamp, cpu_usage_percent, cpu_ctx_switches, cpu_interrupts, cpu_soft_interrupts, cpu_syscalls, mem_total, mem_available, mem_percent, mem_used, mem_free
    </parse>
    <sqs>
      queue_name iac-s3-event-notification-queue
    </sqs>
  </source>
  <match stream_data>
...
#### STEP 3. kinesis streams 구성

- 생성한 kinesis 정보는 아래와 같다


```python
! aws kinesis describe-stream --stream-name=iac-kds

{
    "StreamDescription": {
        ...
        "StreamARN": "arn:aws:kinesis:ap-northeast-2:xxxxxxxxxxx:stream/iac-kds",
        "StreamName": "iac-kds",
        "StreamStatus": "ACTIVE",
...
#### STEP 4. Kinesis to ES 파이프라인 구축을 위한 Lambda 개발
...
def lambda_handler(event,context):
    ...
    for record in event["Records"]:
        ...
        es_id = json_data["timestamp"][0:4]\
                    +json_data["timestamp"][5:7]\
                    +json_data["timestamp"][8:10]\
                    +json_data["timestamp"][11:13]\
                    +json_data["timestamp"][14:16]\
                    +json_data["timestamp"][17:19]
        

        es.index(index="stream_data", doc_type="header_data", id=es_id, body=json_data)
...
def lambda_handler(event, context):
    ...
    for i in range(1,1000):
        ...
        if record['Records']:
            ...
            es.index(index="stream_data_test", doc_type="header_data", id=es_id, body=json_data)
